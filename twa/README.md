...
Одна кривая (`--ease`) и три длительности (`--dur-fast` 120 мс, `--dur` 220 мс,
`--dur-slow` 360 мс) на всё приложение — в `tokens.css`. Правила:

- смена экрана — подъём с проявлением (`.page`);
- элементы списка появляются с задержкой 40 мс друг за другом, но не больше
  шести шагов: иначе хвост длинного списка заметно запаздывает (`.appear`);
- всё нажимаемое отвечает уменьшением (`.pressable`, `.btn`, `.icon-btn`);
- подложка переключателя едет, а не перескакивает;
- шторка уходит тем же движением, каким пришла: `Sheet` отдаёт `close()`
  в children, поэтому кнопки внутри закрывают её с анимацией, а не мгновенно;
- сердце «пикает» при добавлении в избранное, баланс подпрыгивает при изменении.

Системная настройка «уменьшить движение» глушит всё это одним правилом в
`components.css` — проверено, длительности падают до нуля.
